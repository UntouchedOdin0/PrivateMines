...
public class CuboidRegion extends Overlappable {
  ...
  public static CuboidRegion cubeRadius(Location loc, int radius) {
    return new CuboidRegion(loc.clone().subtract(radius, radius, radius),
   ...
   * @return Itself
   */
  public CuboidRegion expand(double posX, double negX, double posY, double negY, double posZ,
      double negZ) {
   ...
   * {@link CuboidRegion#fromString(String)} later
   *
  ...
  public static CuboidRegion fromString(String input) {
    ...
    return new CuboidRegion(new Location(world, minX, minY, minZ),
